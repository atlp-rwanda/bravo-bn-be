...
export const getSingleTripRequest = async (req, res) => {
    ...
    try {
        ...
        if (req.user.role == 'requester') {
            const trips = await tripRequests.findOne({ where: { requesterId: userId, } })
...
export const updateTripRequest = async (req, res) => {
    try {
        ...
        if (tripRequest.status == 'pending') {
            const type = req.body.returnDate == ' ' ? 'one way trip' : 'return type trip';
            const updatedTrip = {
                leavingFrom: req.body.leavingFrom,
                goingTo: req.body.goingTo,
                travelDate: req.body.travelDate,
            ...
            tripRequest.update(updatedTrip, { where: { id: requestId } })
                .then(num => {
                    if (num == 1) {
...
export const deleteTripRequest = async (req, res) => {
    try {
        ...
        if (tripRequest.status == 'pending') {
            await tripRequests.destroy({ where: { id: requestId } })
